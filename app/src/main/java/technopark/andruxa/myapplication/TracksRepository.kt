...
class TracksRepository(private var api: Api?) {
    ...
    private fun search(progress: MutableLiveData<SearchProgress>, query: String, limit: Int) {
        ...
        api?.search(query, limit)?.enqueue(object : Callback<TrackApi.SearchResponse> {
            override fun onResponse(
                    call: Call<TrackApi.SearchResponse>?,
                    response: Response<TrackApi.SearchResponse>
            ) {
            ...
            override fun onFailure(call: Call<TrackApi.SearchResponse>?, t: Throwable?) {
